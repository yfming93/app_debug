...
class AppDebug {
  ...
  static void showDebugTool(BuildContext context, {bool isManuel = false, bool showAnimation = true}) async {
    if (isManuel == false) {
      await getConfig();
      if (isOpenDebug == true){
  ...
  static void snackBar (BuildContext ctx, String text,{String title = "", bool autoCopy = false}){
        ...
        SnackBar(content:  Column(
          ...
          children: <Widget>[
            ...
            Row(
                ...
                children: [
                  Icon(Icons.copy_rounded,color: Colors.green,),
                  SizedBox(width: 15,),
                  Text('Copied',style: TextStyle(color: Colors.red,fontSize: 22),),
                ]
            ) : Text(title),
            Text(text)],
        ),
